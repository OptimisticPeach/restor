...
pub use crate::black_box::unit::{
    DynamicResult, ErrorDesc, NonGuardedUnit, StorageUnit, Unit, UnitError,
};
...
impl<'a> BlackBox<'a, Ref<'a, dyn Any>, RefMut<'a, dyn Any>, Box<dyn Any>, RefCellUnit<'a>> {
    pub fn new() -> Self {
        Self {
            data: HashMap::new(),
    ...
    pub fn allocate_for<T: 'static>(&mut self) {
        if !self.data.contains_key(&TypeId::of::<T>()) {
            self.data.insert(
                TypeId::of::<T>(),
                Box::new(NonGuardedUnit::new(StorageUnit::<T>::new())),
            );
        }
    }
    ...
    #[inline]
    fn unit_get<T: 'static>(&'a self) -> DynamicResult<&'a RefCellUnit<'a>> {
        self.data
            .get(&TypeId::of::<T>())
            .map(|x| &**x)
