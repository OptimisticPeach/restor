use super::{BlackBox, Unit, Map, MapMut, Borrowed, MutBorrowed};
use std::ops::Deref;
...
where Borrowed<'a, U>: Map<(dyn Any), T, Func = fn(&dyn Any) -> &T> {
...
impl<'a, T: Any + 'static, U: for<'b> Unit<'b, Owned=Box<dyn Any + 'static>>> Get<'a, U> for &mut T
where MutBorrowed<'a, U>: MapMut<(dyn Any), T, Func = fn(&mut dyn Any) -> &mut T> {
