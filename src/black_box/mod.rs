...
struct BorrowIndex {
    indexes: Rc<BTreeSet<usize>>
}

impl BorrowIndex {
    pub fn new() -> Self {
        Self {
            indexes: Rc::new(BTreeSet::new())
        }
    }

    pub fn insert_if_possible<'a, T: 'a>(&mut self, candidate: T, idx: usize) -> Result<IndexGuard<'a, T>, ()> {
        if self.indexes.contains(&idx) {
            Err(())
        } else {
            Rc::get_mut(&mut self.indexes).unwrap().insert(idx);
            Ok(IndexGuard {
                data: candidate,
...
struct IndexGuard<'a, T: 'a> {
    pub(crate) data: T,
    pub(crate) indexes: Rc<BTreeSet<usize>>,
    pub(crate) idx: usize,
    pub(crate) unused: PhantomData<&'a ()>,
}

impl<'a, T> Drop for IndexGuard<'a, T> {
    fn drop(&mut self) {
        Rc::get_mut(&mut self.indexes).unwrap().remove(&self.idx);
    }
}
